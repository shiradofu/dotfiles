...
require('zen-mode').setup {
  on_open = function()
    before_zen.wins = vim.tbl_filter(
  ...
  on_close = function()
    ...
    if #new_wins > 0 then
      return vim.defer_fn(
        function() vim.api.nvim_set_current_win(new_wins[1]) end,
        0
      )
    end

    local new_tabs = vim.tbl_filter(
      function(tab) return not vim.tbl_contains(before_zen.tabs, tab) end,
      vim.api.nvim_list_tabpages()
    )
    if #new_tabs > 0 then
      return vim.defer_fn(
        function() vim.api.nvim_set_current_tabpage(new_tabs[1]) end,
        0
      )
    end
  end,
